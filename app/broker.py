...
def create_api_versions_response_message(correlation_id, error_code, message_body):
    ...
    throttle_time_ms = 0
    response_body = struct.pack('>h', error_code) + api_key + struct.pack('>h', min_version) + struct.pack('>h', max_version) + struct.pack('>b', 0) + struct.pack('>h', throttle_time_ms) + struct.pack('>b', 0)
