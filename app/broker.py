...
def create_api_versions_response_message(correlation_id, error_code, message_body):
    ...
    throttle_time_ms = 0
    response_body = struct.pack('>h', error_code) + struct.pack('>b',0) + api_key + struct.pack('>h', min_version) + struct.pack('>h', max_version) + b"\x00" + struct.pack('>i', throttle_time_ms) + b"\x00"
