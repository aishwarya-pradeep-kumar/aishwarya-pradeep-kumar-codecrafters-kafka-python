...
def create_api_versions_response_message(correlation_id, error_code, message_body):
    ...
    response_body = struct.pack('>h', error_code) + struct.pack('>b', 2) + api_key + struct.pack('>h', min_version) + struct.pack('>h', max_version) + struct.pack('>b', 2) + struct.pack('>i', 10000)
